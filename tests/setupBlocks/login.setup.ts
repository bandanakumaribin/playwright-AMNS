...
export const clientLogin = async ({ page }: { page: Page }) => {
...
export const vendorLogin = async ({ page }: { page: Page }) => {
...
// let loginOTP = "987321";
// const clientLogin = async ({ page,phone}) => {

//   // Login via OTP
//   if (process.env.BUILD_URL != null){
//    await test.info().attach('Video : '+ process.env.BUILD_URL+"/HTML_20Report/index.html#?testId="+(test.info().testId) +" ");
//   }
//   await page.getByPlaceholder("Enter Your Phone Number").fill(phone);
//   await page.getByPlaceholder("Enter Your Phone Number").press("Enter");
//   await page.getByPlaceholder("OTP").fill(loginOTP);
//   await page.getByPlaceholder("OTP").press("Enter");
  //await page.getByText("Not now").click();

  // Check if the user is logged in and the menu is visible
  //let modulesToCheck = clientVsModules[type];
  //for (let module of modulesToCheck) {
   // await expect(page.getByRole("menuitem", { name: module })).toBeVisible();
  //}
...
// export { clientLogin ,  vendorLogin ,clientLogout };
